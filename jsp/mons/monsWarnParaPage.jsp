...
<tr>
		...
		<tr>
		    <th>系统编号：</th>
			<td><select name="monType" id="l_monType" class="easyui-validatebox" data-options="required:true"  validType="ccbinsId">
				<option value="" selected="selected">--请选择--</option></select>
				<span style="color:#ff9900">*</span>
			</td>
          
			<th>平均响应时间：</th>
            <td><input name="respTime" type="text" id="l_respTime"  maxlength="50" class="easyui-validatebox" data-options="required:true"/>
            <span style="color:#ff9900">*</span></td>
		</tr>
		<tr>
		  	<th>交易成功率：</th>
          		<td><input name="tranRate" type="text" id="l_tranRate"  maxlength="50" class="easyui-validatebox" data-options="required:true"/>
          		<span style="color:#ff9900">*</span></td>
			<th>业务成功率：</th>
				<td><input name="operRate" type="text" id="l_operRate"  maxlength="50" class="easyui-validatebox" data-options="required:true"/>
            	<span style="color:#ff9900">*</span></td>		
		</tr>  
	</table>
	...
	function dataGridInit(){ 
		dataGridTable=$('#dataGrid').datagrid({//实例化table表单
			url:'<%=basePath%>/WarnParaInit.action',
			...
			columns:[[ //数据表格列配置对象
				...
				{field:'respTime',title:'平均响应时间',width:200},
				{field:'tranRate',title:'交易成功率',width:200},
				{field:'operRate',title:'业务成功率',width:200}
			    ]],
			...
			buttons:[{
				...
				handler:function(){
			    	...
			    	$.ajax({ 
						type: "POST",
						data:ids,
						url:'<%=basePath%>/WarnParaConfirm.action', 
						dataType:"json",
	...
	function initform(rowData) { 
		$('#l_monType').val(rowData.monType);
		$('#l_respTime').val(rowData.respTime);
		$('#l_tranRate').val(rowData.tranRate);
		$('#l_operRate').val(rowData.operRate);
	}
	...
	function init(){
    	...
    	jQuery('#l_monType').append("<option value=''>--请选择--</option>");
    	...
    	$.ajax({ 
			...
			success:function(data){
				for(var prop in data.obj){
					...
					jQuery('#l_monType').append("<option value="
							+data.obj[prop].sysId+">"
							+data.obj[prop].sysName+"</option>");
				}
			}
