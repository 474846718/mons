...
<!-----------------------------表格----------------------------->
	...
	<form id="form1">
		...
		<tr>			
			<th>渠道号：</th>			
			<td><input name="channelId" type="text" id="J_channelId"/></td>
			<th>渠道名：</th>
			<td><input name="channelDesc" type="text" id="J_channelDesc"/></td>
		</tr>
		<tr>
		    <th>系统编号：</th>
...
<!--------------------------录入--------------------------- --> 
  ...
  <tr>
	...
	<form id="J_edit_form">
		...
		<tr>
			<th>渠道号：</th>
			<td><input name="channelId" type="text" id="l_channelId"  class="easyui-validatebox" data-options="required:true" validType="ccbinsId" maxlength="6"/>
				<span style="color:#ff9900">*</span>
            </td>
            <th>渠道名：</th>
			<td><input  name="channelDesc" type="text" id="l_channelDesc" class="easyui-validatebox" data-options="required:true" maxlength="32"/>
			    <span style="color:#ff9900">*</span>
		    </td>
		</tr>         
		<tr>
			...
			</select><span style="color:#ff9900">*</span>
			</td>
		</tr> 
	</table>
	...
	<script type="text/javascript">
        /* 编辑表单 */
		var J_channelId = $('#J_channelId');//渠道号
        var J_channelDesc = $('#J_channelDesc');//渠道名
        var J_sysName = $('#J_sysName');//系统编号
	...
	<script type="text/javascript">
	    ...
	    function dataGridInit(){
	    	dataGridTable = $('#dataGrid').datagrid({//实例化table表单
				url:'<%=basePath%>/ChannelInfoQuery.action',
				...
				selectOnCheck:true,
				sortName:'channelId',//初始化时以那一列来排序
				sortOrder:'asc',//设置排序顺序asc或者desc（正序或者倒序）
				method:'post',
				columns:[[ //数据表格列配置对象
		            {field:'checkbox',checkbox:true}, 
					{field:'channelId',title:'渠道号',width:100,sortable: true}, 
					{field:'channelDesc',title:'渠道名',width:100},
					{field:'sysName',title:'系统名',width:100}
				    ]],  
				onDblClickRow:function(rowIndex,rowData){//当用户双击一行时触发
					    $('#l_channelId').attr("readonly","readonly"); 
    ...
    <script type="text/javascript">
        ...
        function confirmClick(){
        	$('#l_channelId').removeAttr("readonly");
        	$('#l_sysId').removeAttr("disabled");
        	
        	var channelid = $('#l_channelId').val();
        	var channeldesc = $('#l_channelDesc').val();
        	var sysId = $('#l_sysId').val();
        	if(channelid == '' || channelid == null){
        		$.messager.alert("提示信息","请输入渠道号！");
        		return;
        	}
        	if(channeldesc == '' || channeldesc == null){
        		$.messager.alert("提示信息","请输入渠道名！");
        		return;
        	}
        	...
        	$.ajax({ 
        		type: "POST",
    			data:formData,
    			url:'<%=basePath%>/ChannelInfoConfirm.action', 
    			dataType:"json",
    			...
    			buttons:[{
    				...
    				handler:function(){
    					...
    					for(var i in rowDatas){
    						primRows = rowDatas[i].channelId+","+rowDatas[i].sysId+"|"+primRows;
    					}
    ...
    <script type="text/javascript">
        ...
        function initform(rowData){
    		...
    		$('#l_channelDesc').val(rowData.channelDesc);
