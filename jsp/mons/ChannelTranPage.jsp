...
<title>${param.channelName}实时交易统计</title>
...
<body>
<div class="nps_top">
	<div class="nps_top_left"><em><font class="logo_title1">${param.channelName}</font> <font class="logo_title2">实时交易统计</font></em></div>
...
<div class="nps_content">
	<div class="nps_num">
		<ul>
			<li class="nps_num3">成功率 <span id="001">100%</span></li>
			<li class="nps_num2">平均响应时间 <span id="002">0.59s</span></li>
			<li class="nps_num1">交易笔数 <span id="003">3616766</span></li>
		</ul>
	</div>
	<table class="nps_map">
      <tr>
			<td><a href="javascript:cutMenu()">${param.channelName}交易曲线图</a></td>
        	<td><a href="javascript:cutMenu()">${param.channelName}交易笔数</a></td>
	  </tr>
      <tr id="Col_menu">
        <td>
				<div id="detail1"></div>
...
function initFusionCharts1(){
	...
	if ( '${param.Para2}' != "" )
		flag = 2;
	if ( '${param.Para1}' != "" && '${param.Para2}' != ""  )
		flag = 3;
	
	var str="{\"AvgTm_ID\"\:\""+'${param.sysID}'+"\",\"Flag\":\""+
	flag +"\",\"Para1\":\""+
	'${param.Para1}'+"\",\"Para2\":\""+'${param.Para2}'+"\"}";
		...
		success: function(data){
			...
			var begin="<OBJECT width='620' height='225' id='Column3D'>"
				+"<embed src='FusionChartsSWF/MSLine.swf' flashVars=\"&dataXML="
					+"<graph caption='' xAxisName='分钟' rotateNames='1' yAxisName='百分比' outCnvBaseFontColor='000000'"
							+"bgColor='035785,0986c4' bgAlpha='100' canvasBorderColor='ffffff' showNames='1' decimalPrecision='2' formatNumberScale='0' "
							+" drawAnchors ='1' lineThickness='2' showLimots='2'>";
	...
	$.ajax({ 
		...
		success: function(data){
			...
			var begin="<OBJECT width='670' height='225' id='Column3D'>"
                +"<embed src='FusionChartsSWF/Column3D.swf' flashVars=\"&dataXML=<graph caption='交易量/分钟' "
                +"xAxisName='分钟' outCnvBaseFontColor='ffffff'  bgColor='0986c4,035785' rotateNames='1' yAxisName='交易量' showNames='1' decimalPrecision='4' formatNumberScale='0'>";
		...
		success: function(data){
			...
			if(len>0){
			  for(var i=0;i<len;i++){
				  ...
				  if ( dataObj[i].channelId == '${param.Para1}' )
					  channel.add(new Option(dataObj[i].channelDesc, dataObj[i].channelId , 1 , 1 ));
				  else
...
function initbusiType(){
	...
	$.ajax({ 
		...
		success: function(data){
			...
			if(len>0){
			  for(var i=0;i<len;i++){
				  if ( dataObj[i].busiType == '${param.Para2}' )
					  busiType.add(new Option(dataObj[i].busiDesc, dataObj[i].busiType,1,1));
				  else
...
function dataGridInit(){ 
	dataGridTable=$('#dataGrid').datagrid({//实例化table表单
		...
		columns:[[ //数据表格列配置对象
	        ...
	        {field:'RespTime',title:'系统平均响应时间',width:200,sortable: true},
