...
<body>
	...
	<div class="nps_top">
		<div class="nps_top_left">
			<em><font class="logo_title1">${param.sysName}</font> <font
				class="logo_title2">机构业务量统计</font></em>
		</div>
	...
	<div class="nps_content">
		...
		<table class="nps_map">
			<tr>
			    <td><a href="javascript:cutMenu()">机构实时交易量排名 </a></td>
        		<td><a href="javascript:cutMenu()">机构交易笔数</a></td>
			</tr>
			<tr id="Col_menu">
				<td>
		...
		<div id="tb"
			...
			<table class="table_form3">
				...
				<tr>
					...
					<th>机构：</th>
					<td>
						<select id="J_BrchNo" class="easyui-combobox" data-options="multiple:'true',valueField:'brchNo',textField:'brchDesc'" name="brchNo" style="width:150px">
						</select>
					</td>
					<td>
					</td>
	...
	<script type="text/javascript">
		...
		function searchFun() { 
			...
			jCotcode=J_brchCode.combobox('getValue');
			if(jTimeBg >=jTimeEd)
			{
				alert("请选择正确的时间段");
				return;
		...
		function RefreshMON026(){
			var param = "";
			param = "MONSTranNO=MON026&jsonStr={\"AvgTm_ID\":\""+'1'+"\"}";
			$.ajax({ 
				type: "POST",
	...
	function initFusionCharts0(){
		...
		*/
		...
		var str="{\"Top\"\:\""+topNum+"\"}";
		var param = "MONSTranNO=MON027&jsonStr="+str;
		$.ajax({ 
			type: "POST",
			url:'<%=basePath%>/MONSTran.action',
			data:param,
			...
			success: function(data){
				...
				var begin="<graph caption='业务量/机构号' xAxisName='机构号' outCnvBaseFontColor='000000'  rotateNames='1'   bgColor='0986c4,035785' yAxisName='交易量' "
	...
	function initFusionCharts1(){
		...
		var param = "MONSTranNO=MON028&jsonStr="+str;
		$.ajax({ 
			type: "POST",
			url:'<%=basePath%>/MONSTran.action',
			data:param,
			...
			success: function(data){
				...
				var len=dataObj.length;
				var lip = len/30;
				var strlip = "labelStep='"+lip+"'";
				
				
				var begin="<graph caption='' xAxisName='分钟' rotateNames='1' yAxisName='业务量'  outCnvBaseFontColor='000000' labelDisplay='Rotate' slantLabels='1'  labelStep='4' showValues='0'"
								+"  bgColor='035785,0986c4' bgAlpha='100' canvasBorderColor='ffffff' showNames='1' decimalPrecision='2' formatNumberScale='0' "
								+" drawAnchors ='1' baseFontColor='000000' anchorRadius='1' lineThickness='1' showLimits='2' animation='1'>";
				var end="</graph>";
				var coloum="<categories>";
