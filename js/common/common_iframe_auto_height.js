...
var iframeids=["indexFrame","menu_tree"]; 
		...
		if (document.getElementById) { 
			...
			if (dyniframe[i] && !window.opera) { 
				...
				if(dyniframe[i].id == "menu_tree"){
					dyniframe[i].height = document.documentElement.clientHeight - 240;
					//dyniframe[i].height = window.screen.availHeight - 165;//dyniframe[i].contentDocument.body.offsetHeight; 		
				}else if(dyniframe[i].id == "indexFrame"){
					dyniframe[i].height = document.documentElement.clientHeight-35 ;
