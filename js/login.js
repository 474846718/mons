  $(function() {
      var _div=$("#login_photo");
      var _li_photo=$("#login_photo").find("ul>li");     //图片
      var _li_photo_button=$("#login_photo_button").find("ul>li"); //按钮         
      var _li_photo_button_len=_li_photo_button.length;
      var index=1,_Interval;
     function showPhoto(_num) {
        _li_photo_button.eq(_num).addClass("selected").siblings("li").removeClass("selected");
        //_li_photo.eq(_num).animate({opacity:1,'z-index':1}).siblings("li").animate({opacity:0,'z-index':0});  
        _li_photo.fadeOut(1500).eq(_num).fadeIn(1500);
     }
     function ziDong(){  //自动切换
        if(index==_li_photo_button_len) { index=0;}
        showPhoto(index);
        index=index+1;
       }
       _Interval=setInterval(ziDong,3000);
       _li_photo_button.click(function(){
            var index_num=_li_photo_button.index(this);
            showPhoto(index_num);
            index=index_num+1;  //改变全局变量的值，以便鼠标移开的时候能够衔接上 
     });
      _div.mouseover(function(){ //移动到上面时停止自动切换
         clearInterval(_Interval);
       });
      _div.mouseout(function(){ //移开时继续自动切换
         _Interval=setInterval(ziDong,3000);
       });
  })
...
adtec.login.login.isEmpty=function(formIndex){ 
	...
	if(formIndex==0){  
	  if( $.trim(_form.username.value)==""){  
			$("#error"+formIndex).html("用户名不能为空!");            
	        return false;     
	    }else  if ( $.trim(_form.password.value) == "") {
			$("#error" + formIndex).html("密码不能为空!");
			return false;
