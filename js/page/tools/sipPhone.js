...
//	var vVideoSize ="cif"; //默认是384 bps
	...
	var InitialAudio = function() {
		...
		for ( var i = 0; i < vMicCount; i++) {
			...
			if (vCurDevice == vDeviceName) {
		       $E('micDevice').selectedIndex = $E('micDevice').options.length - 1;
			}
		}
		...
		for (i = 0; i < vSpeakerCount; i++) {
			...
			$E('speakerDevice').options.add(new Option(vDeviceName,
					vDeviceName));
			if (vCurDevice == vDeviceName) {
				$E('speakerDevice').selectedIndex = $E('speakerDevice').options.length - 1;
			}
		}
	}
	...
	var InitialVideo = function() {
		...
		for ( var i = 0; i < vMicCount; i++) {
			...
			$E('videoDevice').options
					.add(new Option(vDeviceName, vDeviceName));
			if (vCurDevice == vDeviceName) {
				$E('videoDevice').selectedIndex = $E('videoDevice').options.length - 1;
			}
		}
		InitialVideoSize();
	}
	...
	var InitialVideoSize = function() {
		...
		for ( var i = 0; i < vMicCount; i++) {
			...
			$E('id_videoSize').options
					.add(new Option(vDeviceName, vDeviceName));
			if (vCurDevice == vDeviceName) {
				$E('id_videoSize').selectedIndex = $E('id_videoSize').options.length - 1;
			}
		}
	...
	/*
	 * ---------------初始化-------------------
	 * SetAuthCode(string name,string codec)::注册证书      RETURN::null
	 * 
	 ...
	 * 
	 * ---------------音频API-------------------
	 * Int Audio_GetMicDeviceCount()::获取本地麦克风的个数
	 * String Audio_GetCurrentMicDeviceName()::获取当前麦克风设备名字
	 * String Audio_GetMicDeviceName(int iIndex)：：获取对应的麦克风设备名字
	 * 
	...
	_this.initial = function() {
		if (vInital == true)
			return;
		SipPhoneCtrl.SetAuthCode(certificateName,
				certificateCode);
